...
describe('Style Modifiers and Long Text', () => {
  ...
  it('should handle long text with styles correctly', () => {
    const longText = 'This is a very long text with multiple styles applied to it. ' +
  ...
  it('should handle nested styles correctly', () => {
    const styledText = style
      ...
      .apply(
        'Bold Red ' +
        new Style().italic.fg(Color.blue).apply('Italic Blue ') +
        'Bold Red again'
      );
    const expectedRed = Color.red.code;
    const expectedBlue = Color.blue.code;
    expect(styledText).toBe(`\x1b[1m\x1b[38;${expectedRed}mBold Red \x1b[0m\x1b[3m\x1b[38;${expectedBlue}mItalic Blue \x1b[0m\x1b[1m\x1b[38;${expectedRed}mBold Red again\x1b[0m`);

  });

  it('should handle reset modifiers correctly in long text', () => {
    const longText = 'This is some styled text. ';
    const resetText = 'This should be reset.';
    const styledText = style.bold.fg(Color.green).apply(longText + style.reset.apply(resetText));

    expect(styledText).toBe(`\x1b[1m\x1b[38;5;2m${longText}\x1b[0m${resetText}\x1b[0m`);
  });
  ...
  it('should handle hidden and reveal in long texts', () => {
    ...
    const longText = 'Some visible text. ' + hiddenText + ' Some more visible text. ' + revealedText;
    expect(longText).toBe('Some visible text. \x1b[8mHidden Part\x1b[0m Some more visible text. \x1b[28mRevealed Part\x1b[0m');
  });

  it('should handle multiple foreground and background changes', () => {
    const text = style.fg(Color.red).bg(Color.blue).apply('Red on Blue ') +
    ...
    expect(text).toBe(`\x1b[38;${expectedRed}m\x1b[48;${expectedBlue}mRed on Blue \x1b[0m\x1b[38;${expectedYellow}m\x1b[48;${expectedMagenta}mYellow on Magenta\x1b[0m`);
  });
});